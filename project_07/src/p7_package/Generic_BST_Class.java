...
public class Generic_BST_Class< GenericData extends Comparable< GenericData > >
{
   ...
   {
      /*
       * -do not have to check for left child because we are looking for max
       * value
       * 
       * -should store maxLoc becore it is unlinked
       * 
       * -extraneous code: do not have to set rightChildRef to null; already
       * handled above it
       */
      if ( maxLoc != null )
      {
   ...
   {
      /*
       * -should link result of removeItemHelper to BST_Root
       * 
       * -does not properly remove node when left or right child is null
       * 
       * -does not properly remove node when both children exist
       */
      if ( !isEmpty () )
      {
         GenericData foundValue = search ( inData );
         if ( foundValue != null )
         {
   ...
   private BST_Node removeItemHelper ( BST_Node localRoot, GenericData outData )
   {
      ...
      else
      {
         ...
         if ( temp.nodeData.compareTo ( localRoot.rightChildRef.nodeData )
               == 0 )
         {
